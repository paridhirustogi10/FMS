...
!$OMP END CRITICAL
  ...
  if(need_compute) then
     if(PRESENT(interp)) then
        ...
        deallocate(mask_out)
        !field%need_compute(ib, window_id) = .false. !comment to speed up the
        !bgc computation, liao made according to Andrew's suggestion.
     else
        if ( field%region_type .NE. NO_REGION ) then
     ...
     field%need_compute(ib, window_id) = .false. !add here speed up the
        !bgc computation, liao made according to Andrew's suggestion.
  endif
